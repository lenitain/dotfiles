...
# 先关旧：撤掉上一条 Battery 通知，再弹新的。
# 电量的"弹窗"是 mako 通知，没有窗口可杀，用"先撤旧通知"与时间/音量/蓝牙的
# popup-wrapper（先杀后开）对齐语义。makoctl 的 JSON 键名是下划线 app_name。
makoctl list -j 2>/dev/null \
    | jq -r '.[] | select(.app_name == "Battery") | .id' \
    | xargs -r -I{} makoctl dismiss -n {} 2>/dev/null
