...
# ==================== 窗口缓冲：强制 8bit/通道（修屏幕偏色） ====================
# 现象：连写死的颜色也会偏——#2D353B 在屏幕上量到 #29353A、#232A2E 量到 #212831。
# 原因：Qt 拿 QSurfaceFormat 的通道位宽去 eglChooseConfig，位宽没指定（-1）时 Mesa 会把
# 第一个可用 config 给成 RGB565（16bit），窗口表面于是 R/B 只有 5bit、G 只有 6bit
# （步进 8 / 4），任何颜色提交前都被量化。只有走 Qt Quick 的窗口受影响：QtWebEngine
# 的视图是 QQuickWidget，所以整个 qutebrowser 窗口（含 Qt 画的标签栏/状态栏）都中招；
# 普通 Qt Widgets 窗口用 shm XRGB8888，不受影响。
# 证据：WAYLAND_DEBUG=1 抓包，修复前窗口 dmabuf 是 RG16(DRM_FORMAT_RGB565)、
# 修复后是 XR24(XRGB8888)；
# 参考：同类问题 https://github.com/FreeCAD/FreeCAD/issues/23830
#      （上游 Qt 无全局开关，各 app 只能在创建 QApplication 前自己请求）
# 注意：必须在 QApplication 创建之前生效——config.py 的执行时机正好在其之前。
from qutebrowser.qt.gui import QSurfaceFormat as _QSurfaceFormat

_fmt = _QSurfaceFormat.defaultFormat()
_fmt.setRedBufferSize(8)
_fmt.setGreenBufferSize(8)
_fmt.setBlueBufferSize(8)
_fmt.setAlphaBufferSize(8)  # alpha 会顺带把 EGL config 从 RGB565 这类老配置上推开
...
# 竖排标签栏宽度：刚好放得下 favicon。
# 由源码 TabBarStyle._tab_layout 的扣减顺序决定——padding 和 indicator 都是从
# 标签宽度里先扣掉，不是加在图标之外的：
#   icon 左边缘 x = padding.left(8) + indicator.width(4) + indicator.padding 右(4) = 16
#   再加上 iconSize().width()(28) = 44，右边留 2px → 46。
# 实测 37px 时 icon_rect.right=43 > tab.right=36，图标被切掉 7px（劈成两半）。
c.tabs.width = 46
# 只显示 favicon：format 留空，标签里不画任何文字
c.tabs.title.format = ""
c.tabs.title.format_pinned = ""
# 禁止省略号。"right"（默认值）在文字放不下时会画 …，一个标签一个。
...
# 用绝对路径：相对文件名 "dark-bg.css" 在这一版会校验失败
# （Invalid value 'dark-bg.css' - expected a value of type list but got str），
# 一旦抛异常，本行之后的所有设置都不会生效。
from pathlib import Path as _Path

c.content.user_stylesheets = [str(_Path.home() / ".config/qutebrowser/dark-bg.css")]
