...
description: AI/ML work in scientific domains (biology, chemistry, physics, astronomy, climate, genomics, drug discovery, protein design, single-cell, materials science, medicine, etc.). Discovers and uses Hugging Face scientific datasets (via `datasets`), models (via `transformers`/HF Inference API), Spaces (via `gradio_client`), and blog posts. Trigger on any scientific ML task or resource discovery request.
metadata:
